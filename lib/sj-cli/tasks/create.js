...
module.exports = function (sj) {
  ...
  tasks.create_project = function (task, params, options) {
    ...
    sj.gruntInit('project', initOpts, env, function (err, stdout, stderr) {
      if (stdout.length) sj.print(stdout)
      if (stderr.length) sj.print(stderr)
      ...
      if (!options.install) {
        sj.print('Run ' + 'npm install'.green + ' now to install dependencies before generating any more project components or starting the server.\n')
      }

      if (!env.sj_server) {
        sj.print('Before starting development, you will need to choose one of the ' + 'server-*.js'.bold + ' files in the root of your project and rename it to ' + 'server.js'.bold + '.\n')
      }
  ...
  tasks.create_model = function (task, params, options) {
    ...
    if (name !== classname && !options.force) {
      sj.print('The given model name '.yellow + name.cyan + ' looks non-standard. It has been changed to '.yellow + classname.cyan)
  ...
  tasks.create_controller = function (task, params, options) {
    ...
    if (name !== classname && !options.force) {
      sj.print('The given controller/router name '.yellow + name.cyan + ' looks non-standard. It has been changed to '.yellow + classname.cyan)
