...
module.exports = function (sj) {
  ...
  tasks.help = function (task, params, options) {
    ...
    if (sj.getTask(task, subtask) && sj.getTask(task, subtask).help) {
      ...
      sj.println((task + ' ' + (subtask || '')).green + ln)
      sj.println(help.description + ln)

      if (help.extended) {
        sj.println(help.extended + ln)
      }
      ...
      if (task) {
        sj.println('Invalid command or no help available for '.yellow
          + (task + ' ' + (subtask || '')).green
        )
        sj.println()
      }

      sj.println('To get help for a specific task, use ' + (sj.scriptname + ' help <task> [subtask]').green)
      sj.println('For a list of avaialble tasks, use ' + (sj.scriptname + ' list [task]').green)
      sj.println()
    }
