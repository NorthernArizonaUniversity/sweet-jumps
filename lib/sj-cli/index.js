...
var print = function (s, cb) {
      var ok = process.stdout.write(s || '')
  ...
  , println = function (s, cb) { return print((s || '') + ln, cb) }
  ...
  if (Array.isArray(choices)) {
    message += ' ('.grey
      + _.map(choices, function (op) {
          //return String(op) === String(def) ? String(op).green : String(op).cyan
          return String(op).cyan
        }).join(', ')
      + ')'.grey
  }

  if (def) {
    message += ' [default: '.grey + String(def).green + ']'.grey
  }
...
sj.prompt = function (message, opts, callback) {
  ...
  println(message, function () {
    promptly.prompt(prompt, opts, callback)
  })
}
...
sj.confirm = function (message, opts, callback) {
  ...
  println(message, function () {
    promptly.confirm(prompt, opts, callback)
  })
}
...
sj.choose = function (message, choices, opts, callback) {
  ...
  println(message, function () {
    promptly.choose(prompt, choices, opts, callback)
  })
}
...
sj.password = function (message, opts, callback) {
  message = sj._decoratePrompt(message, null, opts)
  println(message, function () {
    promptly.password(prompt, opts, callback)
  })
}
